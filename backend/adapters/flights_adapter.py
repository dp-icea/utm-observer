...
from ports.airspace_repository import FlightDataPort
from schemas.flights import Flight, QueryFlightsRequest, QueryFlightsResponse
from schemas.external.dss.remoteid import SearchIdentificationServiceAreasResponse
from domain.value_objects import Time, TimeFormat
...
class FlightsAdapter(FlightDataPort):
    ...
    def __init__(self):
        ...
        if not self.base_url:
            raise ValueError("BRUTM_BASE_URL must be set in environment variables")
        if not self.api_key:
    ...
    async def get_active_flights(self, area: QueryFlightsRequest) -> List[Flight]:
        ...
        for isa in isas.service_areas:
            try:
                isa_flights = await self._get_flights_from_isa(isa, query_params)
                flights.extend(isa_flights)
            except Exception as e:
                print(f"Error querying flights for ISA {isa.uss_base_url}: {e}")
                errors.append({
                    "error": str(e),
    ...
    async def _query_identification_service_areas(self, query_params: dict) -> SearchIdentificationServiceAreasResponse:
        """Query ISAs from DSS - moved from DSSRemoteIDService"""
        area = ",".join([
            query_params["lat1"], query_params["lng1"],
        ...
        latest_time = (now + timedelta(seconds=10)).isoformat().replace("+00:00", "") + 'Z'
    ...
    async def _get_flights_from_isa(self, isa, query_params: dict) -> List[Flight]:
        ...
        uss_client = AuthAsyncClient(
            base_url=isa.uss_base_url,
            aud=HttpUrl(isa.uss_base_url).host
        )
        ...
        try:
            ...
            for flight_data in flight_response["flights"]:
                ...
                try:
                    ...
                    print(f"Error fetching flight details for {flight_data['id']}: {e}")
                ...
                flight_obj = Flight(
                    ...
                    details=details_response.get("details") if details_response else None,
                )
