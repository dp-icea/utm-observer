...
const mockDrones: DroneInfo[] = [
  ...
  },
  {
    ...
    status: 'active',
    position: { lat: 40.7580, lng: -73.9855, alt: 200 },
    battery: 67,
    signal: 88,
...
export const DroneTracking = () => {
  ...
  const [selectedProviders, setSelectedProviders] = useState<string[]>(['SkyDrone Corp', 'DroneLogistics']);

  const providers = Array.from(new Set(drones.map(d => d.provider)));
  const filteredDrones = drones.filter(d => selectedProviders.includes(d.provider));
  const activeDrones = filteredDrones.filter(d => d.status === 'active').length;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Radio className={`h-4 w-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
        ...
        {filteredDrones.map((drone) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge 
                variant={drone.status === 'emergency' ? 'destructive' : 'secondary'}
